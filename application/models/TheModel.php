...
class TheModel extends CI_Model{
	...
	public function check_username($username){
		...
		$data = $this->db->get_where('profile',['username' => $username])->num_rows();
		if($data == 0){
			return false;
		}else{
			return true; 
	...
	public function login($data) {
		...
		$result = $this->db->get_where('profile',$data)->num_rows();
		if($result == 1){
			return true;
		}else{
			return false;
	...
	public function get_profile($username){
		...
		$result = $this->db->get_where('profile',['username'=> $username]);
